...
You can force their generation by providing the `--verbose` option to the `mvn` command.

* Main: `target/javac.args`
* Test: `target/javac-test.args`

The following arguments are of particular interest, because they can be required at runtime.
These arguments appear in the `javac-test.args` file only, not in `javac.args`.
