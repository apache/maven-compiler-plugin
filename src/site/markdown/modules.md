...
The Maven 3 way to make a modular project is to put a `module-info.java` file in the root directory of the Java source files.
Because the compilation and execution of tests usually require an amended version of the module information,
Maven 3 allows you to overwrite that file with another `module-info.java` file in the test source directory.
Maven 4 still supports this approach for compatibility reasons.
However, it is deprecated. Support can disappear in a future version.
Migrate to the approach described below.
...
The Maven compiler automatically adds the `--patch-module`, `--add-modules` and `--add-reads` arguments for compiling the tests.
If more `--add-reads` arguments are needed, or if `--add-modules`, `--add-exports` or `--add-opens` arguments are also needed,
then place a `module-info-patch.maven` file (syntax described below) in the `test/java` directory.
...
The directory is always present, even in projects that contain only one module.
More than one Java module can be present in the same Maven sub-project.
Such multi-module projects have advantages. They resolve compiler warnings
in forward references to dependent modules and share test code more easily between modules.
For example, a Maven project for a single Java module named `org.foo.bar` has the following directory layout:
...
That directory level is generated by `javac`. Maven does not invent this convention.

The layout above can be declared with the following fragment in the `pom.xml` file.
This example uses the default directory layout for modular projects.
...
Internal code includes package-private classes, interfaces, methods and fields, and all non-exported packages.
Because the module source hierarchy allows any number of Java modules in the same Maven sub-project,
it is easy to add an `org.foo.bar.test` module which tests the `org.foo.bar` module as if it were
an ordinary client application.


## White Box testing

"White Box testing" refers to tests which have access to the internal classes of the project to test.
For any `<source>` element with the `test` scope, all Java code in the directory managed by that
...
However, when using the module source hierarchy, it is easier to place the test fixtures
